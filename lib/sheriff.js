...
export function reviews(states, minimum = FEATURES.reviews.options.minimum, requestedReviewers = [], compareBranches = []) {
    const [baseBranch, givenBaseBranch] = compareBranches;
...
export function wip(title, pattern = FEATURES.wip.options.pattern) {

    const isSuccess = !minimatch(title, pattern);
