...
export function label(labels, name, compareBranches = []) {
    if (_.isEmpty(name)) {
        name = FEATURES.label.options.name.default;
    }
...
export function branch(branchName, pattern) {
    if (_.isEmpty(pattern)) {
        pattern = FEATURES.branch.options.pattern.default;
    }
...
export function wip(title = '', pattern) {
    if (_.isEmpty(pattern)) {
        pattern = FEATURES.wip.options.pattern.default;
    }
